...

{
  ...
  string nameOfCustomer; // Name of the customer 
   ...
   double costNoDiscount; // Cost before discount applied (if aplicable) 
	...
	if (inFileName !="ship.dat") { cout << "Incorrect File Name" << endl;  // Checks input file name, must be "ship.dat" 
	...
	(finalAmmountDue = 0.00); 
		...
		if ((cusType == 'V') or (cusType == 'R') or (cusType == 'N'))  {cusIsValid = 0;} // checks customer type 
	
		else 
		
		   if (cusType != 'V' || 'N' || 'R')  {cusIsValid = 1;}   
	


	
  if (( packWeight > 0) and (packWeight <=25))  {weightIsValid = 0;}  //Checks package weight 
   
		else  {weightIsValid = 1;} 


	
if ((distanceShipped >= 10) and (distanceShipped <= 3000)) { distanceIsValid = 0;} // Checks distance 
